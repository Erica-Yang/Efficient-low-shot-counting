...
class PositionalEncodingsFixed(nn.Module):
    ...
    def _1d_pos_enc(self, mask, dim):
        ...
        temp = self.temperature ** (2 * (temp.div(2, rounding_mode='floor')) / self.emb_dim) #128

        enc = (~mask).cumsum(dim).float().unsqueeze(-1) / temp #torch.Size([4, 64, 64, 128])
        enc = torch.stack([
    ...
    def forward(self, bs, h, w, device):
        mask = torch.zeros(bs, h, w, dtype=torch.bool, requires_grad=False, device=device) #torch.Size([4, 64, 64])
        x = self._1d_pos_enc(mask, dim=2) #torch.Size([4, 64, 64, 128])
