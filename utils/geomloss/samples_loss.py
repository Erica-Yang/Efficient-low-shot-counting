...
class SamplesLoss(Module):
    ...
    def __init__(self, loss="sinkhorn", p=2, blur=.05, reach=None, diameter=None, scaling=.5, truncate=5, cost=None, kernel=None, cluster_scale=None, debias=True, potentials=False, verbose=False, backend="auto"):
        ...
        self.loss = loss #'sinkhorn'
        self.backend = backend #'tensorized'
        self.p = p #1
        self.blur = blur #0.01
        self.reach = reach #0.5
        self.truncate = truncate #5
        self.diameter = diameter #None
        self.scaling = scaling #0.8
        self.cost = cost
        self.kernel = kernel #None
        self.cluster_scale = cluster_scale #None
        self.debias = debias #False
        self.potentials = potentials #False
    ...
    def forward(self, *args):
        ...
        B, N, M, D = self.check_shapes(l_x, α, x, l_y, β, y) #1,221184,12,2
        ...
        # Run --------------------------------------------------------------------------------
        values = routines[self.loss][backend]( α, x, β, y,   #sinkhorn_tensorized
                    p = self.p, blur = self.blur, reach = self.reach,  #1, 0.01, 0.5
                    diameter=self.diameter, scaling = self.scaling, truncate = self.truncate,  #None, 0.8,5
                    ...
                    verbose = self.verbose ) #3个：0：tensor,1:[1, 221184, 1]; [1,12,1]
    ...
    def process_args(self, *args):
        if len(args) == 6:
            return args
        if len(args) == 4:
            α, x, β, y = args #[1, 221184, 1],[1, 221184, 2]; [1,12,1],[1,12,2]
