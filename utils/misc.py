...
def get_box_map(sample, pos, device, external=False): #([3, 384, 528]);
    box_map = torch.zeros([sample.shape[1], sample.shape[2]], device=device)
    if external is False:
        for rect in pos: #0:[tensor([115]), tensor([309]), tensor([138]), tensor([332])];1:...2:
        ...
        box_map = box_map.unsqueeze(0).repeat(3, 1, 1) #torch.Size([3, 384, 528])
    return box_map
