...
def scaling_parameters( x, y, p, blur, reach, diameter, scaling):
    ...
    if diameter is None:
        D = x.shape[-1] #2
        diameter = max_diameter(x.view(-1,D), y.view(-1,D)) #690.879150390625

    ε   = blur**p #0.01 *1
    ε_s = epsilon_schedule( p, diameter, blur, scaling ) #[52个]
    ρ   = None if reach is None else reach**p #0.5
...
def sinkhorn_loop( softmin, α_logs, β_logs, C_xxs, C_yys, C_xys, C_yxs, ε_s, ρ, 
    ...
    if debias:
        ...
        return None, None, a_y, b_x # torch.Size([1, 12, 1]), torch.Size([1, 221184, 1])
